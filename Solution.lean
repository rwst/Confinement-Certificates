...
* `Z32.residue_not_eventually_constant` and its two variants — Theorem C (`Z32.ResidueCapture`)
* every block-certificate entry — Theorems A, B, D and the `p > q²` table (`Z32.BlockCert`)

The union record of Theorem B is deliberately **not** here: its certificate alone costs about 100
seconds and 12 gigabytes to check, and `Z32/UnionRecord.lean` exists so that nothing else pays
that.  `SolutionRecord.lean` adds it, and only `comparator/record.json` uses that module — so the
other six configs never build it.  See `comparator/README.md`.
