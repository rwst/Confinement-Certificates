...
price, `Z32.union_record_7083_empty` (Theorem B's record, total length `17/24`).  Its certificate
...
The development keeps that cost in a module of its own so that no other file pays it.  Splitting
the solution module the same way preserves that under comparator: only `comparator/record.json`
names `SolutionRecord`, so the other six configs never build `Z32.UnionRecord` at all.
